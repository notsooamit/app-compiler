...
async def _health_check(base_url: str, timeout: int = 30) -> bool:
    """Poll GET / until ANY HTTP response (proof the app is listening). Uses stdlib."""
    ...
    while time.time() < deadline:
        try:
            ...
            return True  # Any response = app is alive
        except urllib.error.HTTPError as e:
            return True  # 4xx/5xx still means app is alive
        except Exception:
...
async def _run_smoke_tests(result: RuntimeResult, config: dict):
    ...
    _skip_prefixes = ("/auth/", "/api/auth/", "/login", "/logout", "/register", "/signup", "/signin")
